...
int sauvegarderFichierCSV(const char *nomFichier, struct Joueur joueurs[], int ligneIndex) {
    ...
    for (int i = 0; i < ligneIndex; i++) {
        if (joueurs[i].Prenom[0] != '\0' && joueurs[i].Nom[0] != '\0') {
            fprintf(fichier, "%d,%d,%s,%s,%s,%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n", 
        joueurs[i].nb, joueurs[i].Num, joueurs[i].Prenom, joueurs[i].Nom, 
        joueurs[i].Team, joueurs[i].Naissance, joueurs[i].Nationalite,
        joueurs[i].P1, joueurs[i].P2, joueurs[i].P3, 
        joueurs[i].Q1, joueurs[i].Q2, joueurs[i].Q3, 
        joueurs[i].RACE);
}
    }
